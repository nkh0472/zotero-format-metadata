...
async function request(
  paperID: string,
  fields: (keyof Result)[] = ["paperId", "title", "externalIds"],
  ...
  return { ...JSON.parse(res.response), _status: res.status };
}
...
export const SemanticScholarService = defineService<Result>({
  ...
  async updateIdentifiers({ identifiers }) {
    if (identifiers.arXiv) {
      const res = await request(parsePaperID("arXiv", identifiers.arXiv));
      if (res._status === 200) {
    ...
    // Since DOI is a reliable identifier that can be properly handled by other services,
    // we do not need to process it here.
    // if (identifiers.DOI) {
    //   const res = await request(parsePaperID("DOI", identifiers.DOI));
    //   if (res._status === 200) {
    //     identifiers.SemanticScholarID = res.paperId;
    //     return true;
    //   }
    // }
    if (identifiers.URL) {
      const res = await request(parsePaperID("URL", identifiers.URL));
      if (res._status === 200) {
        identifiers.SemanticScholarID = res.paperId;
        identifiers.DOI = res.externalIds?.DOI;
        return true;
  ...
  async fetch({ identifiers, debug }) {
    if (!(identifiers.SemanticScholarID || identifiers.DOI))
      return;

    let paperID: string;
    if (identifiers.DOI)
      paperID = parsePaperID("DOI", identifiers.DOI);
    else if (identifiers.SemanticScholarID)
      paperID = parsePaperID("SemanticScholarID", identifiers.SemanticScholarID);
    else
      return;
